...
import { getRankingFor, getRankingByRace, DIST_VARIANTS } from './ranking-raw.js';
...
export async function rankingRoutes(fastify) {
  ...
  fastify.get('/ranking/city/:city', async (req, reply) => {
    ...
    const variants = DIST_VARIANTS[dist] || [dist];
    const distPlaceholders = variants.map((_, i) => `$${i + 2}`).join(', ');
    const genderParamIdx = variants.length + 2;
    const genderClause = gender && gender !== 'all' ? `AND a.gender = $${genderParamIdx}` : '';
    const params = [city, ...variants];
    ...
    const sql = `
      ...
      WHERE LOWER(TRIM(a.city)) = LOWER(TRIM($1))
        AND r.distance IN (${distPlaceholders})
  ...
  // META — cidades por estado (para filtros)
  fastify.get('/api/meta/cidades', async (req, reply) => {
    const { estado } = req.query;
    if (!estado || estado.length !== 2) return reply.code(400).send({ error: 'estado inválido (2 chars)' });
    const rows = await prisma.$queryRawUnsafe(
      `SELECT DISTINCT city FROM "Athlete" WHERE state = $1 AND city IS NOT NULL AND city != '' ORDER BY city LIMIT 500`,
      estado.toUpperCase()
    );
