...
// DB armazena distâncias em metros ("10000") e às vezes em formato antigo ("10K")
// Esta tabela normaliza ambos os formatos para a mesma chave canônica
export const DIST_VARIANTS = {
  '3K':  ['3000', '3K', '3km', '3KM'],
  '5K':  ['5000', '5K', '5km', '5KM'],
  '6K':  ['6000', '6K', '6km', '6KM'],
  '7K':  ['7000', '7K', '7km', '7KM'],
  '8K':  ['8000', '8K', '8km', '8KM'],
  '10K': ['10000', '10K', '10km', '10KM'],
  '12K': ['12000', '12K', '12km', '12KM'],
  '13K': ['13000', '13K', '13km', '13KM'],
  '15K': ['15000', '15K', '15km', '15KM'],
  '21K': ['21097', '21000', '21K', '21km', '21KM'],
  '42K': ['42195', '42000', '42K', '42km', '42KM'],
};
...
export async function getRankingFor(distance, gender = null, limit = 5000) {
  ...
  try {
    const variants = DIST_VARIANTS[distance] || [distance];
    const distPlaceholders = variants.map((_, i) => `$${i + 1}`).join(', ');
    const genderParamIdx = variants.length + 1;
    const genderClause = gender ? `AND a.gender = $${genderParamIdx}` : '';
    const sql = `
      ...
      WHERE r.distance IN (${distPlaceholders})
        AND r."time" > '00:07:00'
        AND r."time" != 'DNS'
